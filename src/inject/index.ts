...
import { BEWLY_PAGE_WORLD_GLOBAL_KEY } from '~/constants/pageWorld'
...
if (isElectronEnv) {
  ...
  const VERSION = typeof __BEWLY_VERSION__ !== 'undefined' ? __BEWLY_VERSION__ : 'dev'
  const globalState = (window as any)[BEWLY_PAGE_WORLD_GLOBAL_KEY] as BewlyPageWorldState | undefined

  if (globalState) {
    notifyExistingPageWorldState(globalState)
  }
  else {
    ...
    ;(window as any)[BEWLY_PAGE_WORLD_GLOBAL_KEY] = state
    installPageWorldPingListener(state)
    ...
    try {
      ...
      // Mark state as ready and notify content script
      state.status = 'ready'
      sendPageWorldReady('initial', VERSION)
    }
    catch (error) {
      state.status = 'failed'
      sendPageWorldFailed(error, VERSION)
    }
  } // end of first-execution guard
}
